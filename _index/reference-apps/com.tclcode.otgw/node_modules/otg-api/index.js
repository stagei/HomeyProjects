...
module.exports = class OTGateway extends EventEmitter {
  ...
  openPort(ip, port) {
    ...
    this.found = false;
		...
		if (ok) {
			...
			this.line = rl.createInterface(this.client, this.client)
			this.line.on('line', (data) => {
				clearTimeout(this.waitTimer)
				otgProcessData(this, data)
			})

      // Error handler; likely the OTG was not found on the given ip:port
      this.line.on('error', (err) => {
        this.debug(API_DBG, 'Socket error:', err)
			...
			this.client.on('close', () => {
				this.debug(API_DBG, 'Connection closed (found =', this.found + ')');
				if (this.found) {
   ...
   */
    ...
    return new Promise((resolve, reject) => {
        ...
        if (this.fwMajorVersion >= 4) { // SR command only available from FW4
            ...
            .catch(err => reject(new Error('Error setting date')))
  			}
  ...
  requestSettings() {
  	for (let i = 0; i < otg.Startup.length; i++) {
          ...
          .catch(err => {
            this.debug('Error with command', cmd, err);
            //i--;
          })
  		}
  	}
   ...
   */
  search() {
  	if (!this.found && this.searchCnt++ < 3) {
        ...
        .catch(err => {
          this.debug('Error getting OTG version information');
          this.search();
        })
  	} else {
  		this.emit('found', { found: this.found })
  	}
  }
        ...
        this.writeCommand(updates[i].set) // update the setting
          .then(val => {
            this.writeCommand(updates[i].check, updates[i].resp) // read the setting
              .catch(err => this.debug('Error reading update', update[i].check, err))
          })
          .catch(val => this.debug('Error setting update', update[i].set, err))
      }
		}
	}
    ...
    if (age > 10) {
      ...
      if (cnt < 5) {
        ...
        context.writeCommand(i)
          .catch(err => context.debug('Error resending command', err))
      } else {
        // Give up
...
function otgRegisterListeners(context) {
  ...
  context.on('msg:VER', (version) => {
    ...
    context.writeCommand('PS=0')
      .catch(err => context.debug('Error requesting updates (PS=0)', err))
